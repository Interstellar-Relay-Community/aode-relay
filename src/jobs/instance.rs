    ...
    async fn perform(self, state: JobState) -> Result<(), Error> {
            ...
            .await
        {
            ...
            Err(e) if e.is_not_found() => {
                ...
                match state
                    .requests
                    .fetch_json_msky::<MskyMeta>(msky_meta_uri.as_str())
                    .await
                {
                    Ok(meta) => {
...
mod tests {
    ...
    #[test]
    fn deser_misskey_instance_without_some_data() {
        let meta: MskyMeta = serde_json::from_str(MISSKEY_BARE_INSTANCE).unwrap();
        assert!(meta.icon_url.is_none());
    }

    #[test]
    fn deser_misskey_instance_with_some_data() {
        let meta: MskyMeta = serde_json::from_str(MISSKEY_STELLA_INSTANCE).unwrap();
        assert_eq!(
